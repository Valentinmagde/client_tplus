...
<div class="layout--main" :class="[layoutTypeClass, navbarClasses, footerClasses, {'no-scroll': isAppPage}]">
    ...
    <v-nav-menu @currentDashboard="route = $event" :navMenuItems="navMenuItems" :navMenuItemsAdmin="navMenuItemsAdmin" title="Tontine.Plus" parent=".layout--main" />
    ...
    <div id="content-area" :class="[contentAreaClass, {'show-overlay': bodyOverlay}]">
        ...
        <template v-else>
            <the-navbar-vertical :key="route" :navbarColor="navbarColor" :class="[
          {'text-white' : isNavbarDark  && !isThemeDark},
          {'text-base'  : !isNavbarDark && isThemeDark}
        ]" />
        </template>
            ...
            <div class="router-view">
                <div class="router-content">
                    ...
                    <transition :name="routerTransition">
                        ...
                        <div v-if="$route.meta.breadcrumb || $route.meta.pageTitle" class="router-header flex flex-wrap items-center mb-base fixed z-50 root-bg-color">
                            ...
                            <div class="vx-breadcrumb ml-4 md:block hidden" v-if="$route.meta.breadcrumb">
                                <ul class="flex flex-wrap items-center">
                                    <li class="inline-flex items-end">
                                        <router-link :to="$route.meta.breadcrumb[0].url">
                                            <feather-icon icon="HomeIcon" svgClasses="h-5 w-5 mb-1 stroke-current text-primary" />
                                        </router-link>
                                        <span class="breadcrumb-separator mx-2">
                                            <feather-icon :icon="$vs.rtl ? 'ChevronsLeftIcon' : 'ChevronsRightIcon'" svgClasses="w-4 h-4" />
                                        </span>
                                    </li>
                                    <li v-for="(link, index) in $route.meta.breadcrumb.slice(1,-1)" :key="index" class="inline-flex items-center">
                                        <router-link :to="link.url" v-if="link.url">{{ $t(link.i18n) }}</router-link>
                                        <span class="text-primary cursor-default" v-else>{{ $t(link.i18n) }}</span>
                                        <span class="breadcrumb-separator mx-2 flex items-start">
                                            <feather-icon :icon="$vs.rtl ? 'ChevronsLeftIcon' : 'ChevronsRightIcon'" svgClasses="w-4 h-4" />
                                        </span>
                                    </li>
                                    <li class="inline-flex">
                                        <span v-if="$route.meta.breadcrumb.slice(-1)[0].active" class="cursor-default">{{ $t($route.meta.breadcrumb.slice(-1)[0].i18n) }}</span>
                                    </li>
                                </ul>
                            </div>
                        </div>
...
export default {
    ...
    data() {
        return {
            ...
            navMenuItems: navMenuItems,
            navMenuItemsAdmin: navMenuItemsAdmin,
...
<style>
.con-pagination-table.vs-table--pagination div.vs-pagination--mb {
    width: 50% !important;
}
</style>
