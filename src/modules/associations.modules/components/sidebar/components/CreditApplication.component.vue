  ...
  <div>
    ...
    <span class="flex">
      ...
      <p class="vs-input--label mt-5 inline-block">
        <ejs-tooltip
          ref="tooltip"
          :content="$t('maximumBorrowingAmountConstraint')"
          position="bottom"
    ...
    <money
      v-model="montant_credit"
      v-bind="money"
      class="money-input pt-3 pb-3 w-full"
      @onchange="validateForms"
    />
    <span class="text-danger text-sm" v-show="maximumAmount">{{
      $t("maximumBorrowingAmountConstraint")
    }}</span>
    ...
    <span class="flex">
      <p class="vs-input--label mt-5 inline-block">
        {{ $t("limitNumberOfMeetings") }}
      </p>
      <p class="vs-input--label mt-5 inline-block">
        <ejs-tooltip
          ref="tooltip"
          :content="$t('maximumNumberOfGeneralMeetingsForReimbursement')"
          position="bottom"
          class="mr-2 inline-block"
        >
          <feather-icon
    ...
    <vs-input
      ...
      @onchange="validateForms"
    />
    <span class="text-danger text-sm" v-if="deadline">{{
      $t("valueMustBeLessThanMaximumReimbursement")
    }}</span>
    <!-- Aperçu des échéances -->
    <vs-button
      id="button-loading"
      class="flex mt-5 pt-2 pb-2"
      color="#3EB2FB"
      :disabled="!formsValidate"
      @click="apercuEcheance"
    >
...
export default {
  ...
  computed: {
    selectOptions() {
      if (this.membres != null)
        return this.membres.map((membre) => ({
          id: membre.id,
          text: this.$options.filters.Capitalize(membre.membre),
    ...
    maximumAmount() {
      if (
        parseInt(this.montant_credit) >
        parseInt(this.mutuelle.maximum_empruntable)
      ) {
        return true;
      } else return false;
    },
  ...
  methods: {
    ...
    apercuEcheance() {
      let payload = {
        resourceUrl:
          "api/mutuelle/" + this.mutuelle.id + "/credit/preview/echeance",
        commitAction: "SET_CREDIT",
        data: {
          mutuelles_activites_id: this.activite.id,
