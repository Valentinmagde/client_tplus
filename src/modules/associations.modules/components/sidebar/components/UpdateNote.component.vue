...
export default {
    ...
    computed: {
        ...
        validateForms() {
            if (this.loader &&
                this.nom != '') return this.$emit('disable', false)
    ...
    watch: {
        note: function (newValue) {
            this.nom = newValue.note
        }
    },

    mounted() {
        this.nom = this.note.note
    },
    methods: {
        /* Loader */
        ...
        submit() {
            ...
            let payload = {
                resourceUrl: 'api/note/'+this.note.id+'/update',
                commitAction: 'SET_NOTE',
                data: {
                    note: this.nom,
