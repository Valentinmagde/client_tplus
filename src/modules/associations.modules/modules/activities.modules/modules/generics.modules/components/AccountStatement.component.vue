<template>
    <div v-if="isReady" id="data-list-list-view" class="data-list-container">
        <div class="ml-4 mr-4">
            <vx-card no-shadow class="mb-6 pb-6" :title="$t('statementOfAccounts')+' '+nom_activite"/>
        ...
        <Data-view-sidebar
            ...
            :activity="activity"
        />
        <vs-table v-if="comptesData.length" ref="table" pagination search :data="comptesData" :max-items="itemsPerPage" class="table mt-4 w-full">
...
export default {
    ...
    methods: {
        ...
        updateCompte(){
            ...
            /* let association_courante = JSON.parse(localStorage.getItem('association_courante')); */
            let activite_courante = this.activity !== '' ? this.activity : this.$store.state.association.activite;
    ...
    created(){
        ...
        if(localStorage.getItem('activity_id') !== null){
            ...
            .then(() => {this.isReady = true; EventBus.$emit('loader', false)})
            .catch((error) => {
