<template>
  <div id="data-list-list-view" class="data-list-container">
    ...
    <div class="pt-16">
        ...
        <div class="flex flex-wrap-reverse items-center data-list-btn-container">
            ...
            <div v-if="admin && activitesData.length == 0" class="btn-add-new p-3 mb-4 mr-4 rounded-lg cursor-pointer flex items-center justify-center text-lg font-medium text-base text-primary border border-solid border-primary"
              @click="redirectCreation">
                ...
                <span class="ml-2 text-base text-primary">{{$t('newGenericActivity')}}</span>
            </div>
          </div>
      ...
      <vs-table v-show="activitesData.length > 0" ref="table" :data="activitesData" pagination :max-items="itemsPerPage" search>
...
export default {
  ...
  computed:{
    ...
    activitesData(){
      return this.$store.state.association.activites_generiques
    },
    currentPage() {
      if(this.isMounted) {
  ...
  methods: {
    ...
    deleteItem(id) {
      ...
      .then(() => {
        ...
        let payload = {
          resourceUrl: '/api/association/'+association_courante.id + '/activite/type/Generique',
          commitAction: 'SET_ACTIVITES_GENERIQUES'
        }
    ...
    redirectDetails(tr){
      ...
      if(this.admin)
        this.$router.push('/association/administration/activities/generic/details').catch(() => {})
      else
        this.$router.push('/association/my-activities/generic/details').catch(() => {})
    },
   ...
   created(){
    ...
    //   }
      ...
      let payload = {
        resourceUrl: '/api/association/'+current_association.id + '/activite/type/Generique',
        commitAction: 'SET_ACTIVITES_GENERIQUES'
      }
      this.$store.dispatch("association/fetchAssociationactivite",payload)
      .then(()=>{
