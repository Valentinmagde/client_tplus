<template>
  <div  id="data-list-list-view" class="data-list-container pt-16">
      ...
      <vs-popup classContent="popup-example"  :title="$t('deleteConfirmTitle')" :active.sync="popupActivo2">
        ...
        <div class="vx-row w-full justify-end">
            ...
            <vs-button
              ...
              @click.stop="deleteItem(tontine_selectionne)">
              {{$t('delete')}}
            </vs-button>
        </div>
      </vs-popup>
      ...
      <div class="z-10">
        <div class="ml-5 mr-5" v-if="tontinesData.length == 0">
          <!-- <vx-card class="mb-5">
          ...
          <div class="flex flex-wrap-reverse items-center data-list-btn-container">
            ...
            <div v-if="admin" class="btn-add-new p-3 mb-4 mr-4 rounded-lg cursor-pointer flex items-center justify-center text-lg font-medium text-base text-primary border border-solid border-primary"
              @click="redirectCreation">
                ...
                <span class="ml-2 text-base text-primary">{{$t('newTontine')}}</span>
            </div>
          </div>
          <vx-card>
        ...
        <vs-table v-show="tontinesData.length > 0" ref="table" :data="tontinesData" pagination :max-items="10" search>
          <div slot="header" class="flex flex-wrap-reverse items-center flex-grow justify-between">
            ...
            <vs-dropdown vs-trigger-click class="cursor-pointer mb-4 mr-4 items-per-page-handler">
              <div class="p-4 border border-solid d-theme-border-grey-light rounded-full d-theme-dark-bg cursor-pointer flex items-center justify-between font-medium">
                <span class="mr-2">{{ currentPage * itemsPerPage - (itemsPerPage - 1) }} - {{ tontinesData.length - currentPage * itemsPerPage > 0 ? currentPage * itemsPerPage : tontinesData.length }} of {{ queriedItems }}</span>
            ...
            <vs-tr :key="ind" v-for="(tr, ind) in data"
              ...
              <vs-td >
                ...
                <vx-tooltip v-if="admin" :text="$t('delete')" position="bottom" class="inline-block">
                  <feather-icon  icon="TrashIcon" svgClasses="w-5 h-5 hover:text-danger stroke-current" @click.stop="popupActivo2=true; tontine_selectionne=tr.id"/>
                </vx-tooltip>
              </vs-td>
            </vs-tr>
          </template>
...
export default {
  ...
  computed:{
    ...
    tontinesData(){
      return this.$store.state.association.tontines
    },
    currentPage() {
      if(this.isMounted) {
    ...
    queriedItems() {
      return this.$refs.table ? this.$refs.table.queriedResults.length : this.tontinesData.length
    }
  },
  methods: {
    ...
    redirect(tontine){
      localStorage.setItem('activity_id', JSON.stringify(tontine.id))

      this.$store.commit('association/SET_ACTIVITE', tontine);

      if(this.admin)
        this.$router.push('/association/administration/activities/tontine/details').catch(() => {})
      else
        this.$router.push('/association/my-activities/tontine/details').catch(() => {})
    },
  ...
  created(){
        ...
        let payload = {
          resourceUrl: '/api/association/'+current_association.id + '/activite/type/Tontine',
          commitAction: 'SET_TONTINES'
        }
        this.$store.dispatch("association/fetchAssociationactivite",payload)
        .then(()=>{
