    ...
    <div v-if="isReady">
      ...
      <div class="pt-16">
        ...
        <vs-tabs :position="isSmallerScreen ? 'top' : 'left'"
          :value="selectedTab" class="tabs-shadow-none z-10" id="profile-tabs">

          <!-- Paramèttre de la mutuelle -->
          <vs-tab :label="!isSmallerScreen ? $t('setting'): ''" icon-pack="streameline" icon="icon-saving-piggy-coins">
            <div id="setting" class="tab-general md:ml-4 md:mt-0 mt-4 ml-0">
              <settings @selectedTab="selectedTab = $event" :activity="activity" :ags="ags"/>
            </div>
          </vs-tab>

          <!-- Inscription des membres -->
          <vs-tab id="slectmembers" :label="!isSmallerScreen ? $t('registeredMembers'): ''"  icon-pack="streameline"
          ...
          <!-- Mise des fonds -->
          <vs-tab id="slectmembers" :label="!isSmallerScreen ? $t('downPayment'): ''"  icon-pack="streameline"
            icon="icon-cash-payment-coin-1">
            <div class="tab-general md:mt-0 mt-4 ml-0">
              <fundings :activity="activity" @selectedTab="selectedTab = $event"/>
            </div>
          </vs-tab>

          <!-- Emprunts en cours -->
          <vs-tab id="slectmembers" :label="!isSmallerScreen ? $t('loansInProgress'): ''"
            icon-pack="streameline" icon="icon-cash-payment-bag">
            <div class="tab-general md:mt-0 mt-4 ml-0">
              <loans-in-progress :activity="activity" @selectedTab="selectedTab = $event"/>
            </div>
          </vs-tab>
          ...
          <vs-tab :label="!isSmallerScreen ? $t('checkoutAdjustment'): ''" icon-pack="streameline"
...
import CheckoutAdjustment from '../components/CheckoutAdjustment.component.vue'
import Fundings from '../components/Fundings.component.vue'
