<template>
  <div  id="data-list-list-view" class="data-list-container pt-16">

      <!-- Popup pour supprimer une mutuelle -->
      <vs-popup classContent="popup-example"  :title="$t('deleteConfirmTitle')" :active.sync="popupActivo2">
        ...
        <div class="vx-row w-full justify-end">
            ...
            <vs-button
              ...
              @click.stop="deleteItem(mutuelle_selectionne)">
              {{$t('delete')}}
            </vs-button>
        </div>
      </vs-popup>
      ...
      <div class="z-10">
        <div class="ml-5 mr-5" v-if="mutuellesData.length == 0">
          <!-- <vx-card class="mb-5">
            <h4  class="mb-2"> {{ $t('listOfTontines') | CAPITALIZE }} </h4>
          </vx-card> -->
          <div class="flex flex-wrap-reverse items-center data-list-btn-container">
              ...
              <div v-if="admin" class="btn-add-new p-3 mb-4 mr-4 rounded-lg cursor-pointer flex items-center justify-center text-lg font-medium text-base text-primary border border-solid border-primary"
                @click="redirectCreation">
                  ...
                  <span class="ml-2 text-base text-primary">{{$t('newCreditSavings')}}</span>
              </div>
            </div>
          <vx-card>
        ...
        <vs-table v-show="mutuellesData.length > 0" ref="table" :data="mutuellesData" pagination :max-items="10" search>
          <div slot="header" class="flex flex-wrap-reverse items-center flex-grow justify-between">
            ...
            <vs-dropdown vs-trigger-click class="cursor-pointer mb-4 mr-4 items-per-page-handler">
              <div class="p-4 border border-solid d-theme-border-grey-light rounded-full d-theme-dark-bg cursor-pointer flex items-center justify-between font-medium">
                <span class="mr-2">{{ currentPage * itemsPerPage - (itemsPerPage - 1) }} - {{ mutuellesData.length - currentPage * itemsPerPage > 0 ? currentPage * itemsPerPage : mutuellesData.length }} of {{ queriedItems }}</span>
          ...
          <template slot-scope="{data}">
            <vs-tr :key="ind" v-for="(tr, ind) in data"
              @click.native="redirect(tr)" class="cursor-pointer">
              <vs-td >
              ...
              <vs-td >
                <!--<vx-tooltip :text="$t('detail')" position="bottom"  class="inline-block mr-2">
                  <feather-icon icon="EyeIcon" svgClasses="w-5 h-5 hover:text-primary stroke-current"  @click="redirect(tr)"/>
                </vx-tooltip>-->
                <vx-tooltip v-if="admin" :text="$t('delete')" position="bottom" class="inline-block">
                  <feather-icon  icon="TrashIcon" svgClasses="w-5 h-5 hover:text-danger stroke-current" @click.stop="popupActivo2=true; mutuelle_selectionne=tr.id"/>
                </vx-tooltip>
              </vs-td>
            </vs-tr>
          </template>
...
export default {
  ...
  computed:{
    ...
    mutuellesData(){
      return this.$store.state.association.mutuelles
    },
    currentPage() {
      if(this.isMounted) {
    ...
    queriedItems() {
      return this.$refs.table ? this.$refs.table.queriedResults.length : this.mutuellesData.length
    }
  },
  methods: {
    ...
    deleteItem(id) {
      ...
      this.$store.dispatch("association/removeRecord", payload)
        .then(() => {
          ...
          let payload = {
            resourceUrl: '/api/association/'+association_courante.id + '/activite/type/Mutuelle',
            commitAction: 'SET_MUTUELLES'
          }
    ...
    //Redirige vers la page d'accueil de la tontine
    redirect(mutuelle){
      localStorage.setItem('activity_id', JSON.stringify(mutuelle.id))

      this.$store.commit('association/SET_ACTIVITE', mutuelle);

      if(this.admin)
        this.$router.push('/association/administration/activities/mutual/details').catch(() => {})
      else
        this.$router.push('/association/my-activities/mutual/details').catch(() => {})
    },
  ...
  created(){
      ...
      let payload = {
        resourceUrl: '/api/association/'+current_association.id+'/mutuelles',
        commitAction: 'SET_MUTUELLES'
      }
      this.$store.dispatch("association/fetchAssociationactivite",payload)
      .then(()=>{
