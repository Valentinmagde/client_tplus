    ...
    <div class="pt-16">
      <div>
        <vx-card class="mb-base">
          <h5>
            {{ $t("deadlines") | CAPITALIZE }} - {{ nom_activite | CAPITALIZE }}
          </h5>
...
export default {
      ...
      if (data.length > 0) {
        data.forEach((element) => {
          if (this.switch2) {
            if (element.length > 0) {
              element.forEach((echeance) => {
                if (echeance.date_limite == this.ags.date_ag) {
                  if (this.switch1) {
                    if (this.membre_id == echeance.membres_id) {
                      if (echeance.debit_credit == "decaissement")
                        cpt += echeance.montant;
                    }
                  } else {
                    if (echeance.debit_credit == "decaissement") {
                      cpt += echeance.montant;
  ...
  methods: {
    ...
    settlementsExport(type = "pdf") {
      ...
      const aggregates = {};
      aggregates[this.$t('totalPayment')] = this.$options.filters.formatMoney(this.credit, this.currentAssociation.devise)
      aggregates[this.$t('totalDisbursement')] = this.$options.filters.formatMoney(this.debit, this.currentAssociation.devise)
        ...
        exports[`${type}Export`](
          {
            ...
            body: this.queriedResults.map((settlement) => [
              settlement.membre,
              settlement.libelle,
