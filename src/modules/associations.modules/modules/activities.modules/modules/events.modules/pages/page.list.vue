<template>
  <div id="data-list-list-view" class="data-list-container">
    ...
    <div class="pt-16">
      <div class="ml-5 mr-5">
        <!-- <vx-card class="mb-5">
            <h4  class="mb-2"> {{ $t('eventList') | CAPITALIZE }} </h4>
        </vx-card> -->
         <div class="flex flex-wrap-reverse items-center data-list-btn-container">
          <!-- ADD NEW -->
          <div v-if="admin && activitesData.length == 0" class="btn-add-new p-3 mb-4 mr-4 rounded-lg cursor-pointer flex items-center justify-center text-lg font-medium text-base text-primary border border-solid border-primary"
            @click="redirectCreation">
              <feather-icon icon="PlusIcon" svgClasses="h-4 w-4" />
              <span class="ml-2 text-base text-primary">{{$t('newEvent')}}</span>
          </div>
        </div>
      ...
      <!-- Tableau des activites -->
        <vs-table v-show="activitesData.length > 0" ref="table" :data="activitesData" pagination :max-items="10" search >
          ...
          <template slot-scope="{data}">
            <vs-tr :key="ind" v-for="(tr, ind) in data"
              ...
              <vs-td>
                <p class="product-name">{{ tr.Evenement.date_event | dateTime }}</p>
              </vs-td>

              <vs-td>
                <p class="product-name">{{tr.Evenement.lieu_event}}</p>
              </vs-td>
              ...
              <vs-td >
                ...
                <vx-tooltip v-if="admin" :text="$t('delete')" position="bottom" class="inline-block">
                    <feather-icon  icon="TrashIcon" svgClasses="w-5 h-5 hover:text-danger stroke-current" @click.stop="popupActivo2=true; activite_selectionne=tr.id"/>
                  </vx-tooltip>
              </vs-td>
            </vs-tr>
...
export default {
  ...
  computed:{
    ...
    activitesData(){
      return this.$store.state.association.events
    },
    currentPage() {
      if(this.isMounted) {
  ...
  methods: {
    ...
    redirectDetails(tr){
      ...
      if(this.admin)
        this.$router.push('/association/administration/activities/event/details').catch(() => {})
      else
        this.$router.push('/association/my-activities/event/details').catch(() => {})
    },
   ...
   created(){
    ...
    //   }
      ...
      let payload = {
        resourceUrl: '/api/association/'+current_association.id + '/activite/type/Evenement',
        commitAction: 'SET_EVENTS'
      }
      this.$store.dispatch("association/fetchAssociationactivite",payload)
      .then(()=>{
