<template>
    <!-- Membres Inscrits-->
    <vx-card no-shadow :title="$t('membershipregistration') +' : '+activity.nom">
        ...
        <vs-checkbox :checked="checkedMembers.length == usersData.length" class="mb-3">{{$t('selectAll')}}</vs-checkbox>
          ...
          <ul class="centerx">
              <li class="vx-col w-1/2 mt-3 inline-block"
                 v-for="member in usersData" :key="member.id"
          ...
          <div class="flex flex-wrapper mt-10">
            <vs-button
              type="border"
              class="mr-6"
              @click.native="previous">{{$t('previous')}}
            </vs-button>
          ...
          this.openLoadingContained()
            ...
            .then(() => {
              ...
              if(localStorage.getItem('activity_id') !== null)
                accounts = this.compteMemberData
