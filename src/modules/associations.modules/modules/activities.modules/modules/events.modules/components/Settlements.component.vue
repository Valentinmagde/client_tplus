<template>
    <div v-if="isReady">
        <div class="ep-association">
            ...
            <v-select
                @change="validateForm"
                :options="selectOptionsFrequence"
                v-model="frequence"
                label="text"
                class="w-full mb-5"/>
            ...
            <ul class="centerx mt-4">
                <li>
                    <vs-radio  v-model="radios" vs-name="radios" @change="validateForm" vs-value="1" class="mb-4">{{$t('allMembers')}}</vs-radio>
                </li>
                <li>
                    <div class="flex flex-wrapper mb-5">
                        <vs-radio v-model="radios" vs-name="radios" @change="validateForm" vs-value="0" class="mr-12">{{$t('selectedMembers')}}</vs-radio>
        ...
        <!-- <div class="flex flex-wrapper mt-5">
            <vs-button type="border" class="mr-6" id="button-loading" @click="this.$parent.previous()">{{$t('previous')}}</vs-button>
    ...
    export default {
        ...
        computed : {
            ...
            selectOptionsFrequence(){
                return deadline_frequence.map(frequence => ({text: this.$t(frequence.i18n), value: frequence.value}))
            },
            validateForm() {
                if(this.montant != '' && this.date_limite_versement !='' && this.frequence !='' && this.radios !='')
                    return this.$emit('formValidate', false)
                else
                    return this.$emit('formValidate', true)
            },
        },
        mounted(){
        ...
        methods: {
            ...
            submit(){
                if(localStorage.getItem('activity_id') !== null){
                    ...
                    var activite_courante =  this.activity !== '' ? this.activity : this.$store.state.association.activite;
                    var urlResource, method, data

                    if(this.frequence.value == 'UNEFOIS') {
                        if(this.radios == '1'){
                            urlResource = "/api/activite/" + activite_courante.id + "/echeancier/evenement"
                            ...
                            data = {
                                ...
                                libelle: this.$t('Contribution') + ' - ' + this.nom_activite + ' - '+ this.description,
                            }
                        }
                        else{
