...
<div>
    <data-view @closeSidebar="toggleDataSidebar" :isSidebarActive="addNewDataSidebar" :data="emptyData">
        ...
        <template slot="component">
            <!-- Titre -->
            <vs-input :label="$t('title')" v-model="rubriqueTitle" class="w-full mb-5" />

            <!-- Description -->
            <p class="vs-input--label">{{$t('description')}}</p>

            <ckeditor :editor="editor" v-model="rubriqueComment" @ready="onReady" :config="editorConfig" style="min-height: 450px"></ckeditor>
        </template>

        <vs-button class="mr-6" type="border" color="danger" @click="isSidebarActiveLocal = false">{{$t('cancel')}}</vs-button>
        <vs-button id="button-loading" :disabled="disable" @click.prevent="saveNewRubrique">{{$t('submit')}}</vs-button>
    </data-view>

    <vs-dropdown v-if="this.rapportId && !admin" vs-trigger-click class="ml-auto md:block cursor-pointer fixed z-50" style="right:35px">
        <vs-button icon="icon-settings" icon-pack="feather">{{$t('actions')}}</vs-button>
        <vs-dropdown-menu class="w-48">
            <vs-dropdown-item>
                <div @click="() => addNewDataSidebar = !addNewDataSidebar " class="flex items-center">
