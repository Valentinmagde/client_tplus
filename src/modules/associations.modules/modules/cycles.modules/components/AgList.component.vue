<template>
    <div>
        <div class="ml-5 mr-5">
            <vx-card no-shadow class="mb-5 pb-6" :title="$t('generalMeetings')"/>
        </div>

        <div class="ml-4 mr-4">
            <vx-card no-shadow v-if="ags.length == 0" class="mb-base">
            ...
            <vs-table ref="table" pagination :max-items="itemsPerPage" search :data="ags">
                <div slot="header" class="flex flex-wrap-reverse items-center flex-grow justify-between">
                    <div class="flex flex-wrap-reverse items-center data-list-btn-container">
                        <!-- ADD NEW -->
                        <vx-tooltip position="bottom" :text="$t('byClickingOnThisButtonYourMissingGeneralMeetingsWillBeRegenerated')">
                            <div
                                ref="loadableButton"
                                id="button-with-loading-patche" 
                                class="vs-con-loading__container btn-add-new p-3 mb-4 mr-4 rounded-lg cursor-pointer flex items-center justify-center font-medium text-base text-primary border border-solid border-primary"
                                @click="patchAgs('button-with-loading-patche')">
                                <!-- <feather-icon icon="PlusIcon" svgClasses="h-4 w-4" /> -->
                                <span class="ml-2 text-base text-primary">{{$t('completeTheMissingGeneralMeetings')}}</span>
                            </div>
                        </vx-tooltip>
                    </div>
                    <!-- ITEMS PER PAGE -->
                    ...
                    <vs-tr :key="index" v-for="(ag, index) in data">
                        ...
                        <vs-td class="whitespace-no-wrap">
                            ...
                            <span v-if="ag.etat=='past'">
                                ...
                                <!-- Voir le rapport -->
                                <vx-tooltip v-else :text="$t('seeTheReport')" position="bottom" class="inline-block"
                            ...
                            <span v-if="ag.etat=='current' || ag.etat=='future'">
                                <vx-tooltip :text="$t('reportCancel')" position="bottom"  class="inline-block"
                                    @click.native="popupActivo1=true; lieu_ag=ag.lieu_ag; date_ag=ag.date_ag*1000;
                            ...
                            <span  v-if="ag.etat=='current'">
                                <vx-tooltip :text="$t('manageSession')" position="bottom" class="inline-block"
                                    @click.native="handleSeance(ag)">
                                    <feather-icon  icon="SettingsIcon" svgClasses="w-5 h-5 hover:text-success stroke-current"/>
                                </vx-tooltip>
                            </span>
...
// import mixins from '@/views/pages/Association/Home/seance/SeanceComponents/mixins/data'
...
export default {
    ...
    methods: {
        patchAgs(idLoader){
            let cycle = this.cycle_actif.cycle
            var current_association = this.$store.state.association.currentAssociation
            let payload = {
                resourceUrl: '/api/association/' + current_association.id + '/cycle/'+ cycle.id +'/ags/patche',
        ...
        uploadReport(ag, index){
            ...
            window.console.log(fileToUpload)
                ...
                if(donnees.data.length > 0) {
                    ...
                    if(cycleActif != null){
                        ...
                        if(cycleActif.ag.length > 0) {
                            this.ags = cycleActif.ag.sort((a, b) => (a.date_ag > b.date_ag) ? 1 : -1)
